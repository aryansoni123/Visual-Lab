"""Solution 1: Strict manifold constraint (cap=2 only, no fallback to 3/4)."""
...
def find_faces_strict_cap2(Lambda, Theta, max_iterations=1000000):
    """Strict manifold: cap=2 only, no fallback."""
    num_v = len(Lambda)
    num_e = len(Theta)
    ...
    def dfs(idx, selected_cycles, edge_incidence, covered_edges):
        ...
        # STRICT: cap=2 only
        if all(edge_incidence[e] < 2 for e in cand_edges):
    ...
    if selected is None:
        print(f"✗ Could not find {F_required} faces with strict cap=2")
        return None
    
    print(f"✓ Found {len(selected)} faces with strict cap=2")
...
def main():
    ...
    faces = find_faces_strict_cap2(Lambda, Theta)
    
    if faces is None:
        print("Failed to find valid face set with strict cap=2")
        return
    ...
    out_path = Path(__file__).resolve().parents[1] / "outputs" / "solution1_strict_cap2.stl"
